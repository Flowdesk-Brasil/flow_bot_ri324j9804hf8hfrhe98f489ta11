...
const { buildWhitelistPanelPayload } = require("../utils/componentFactory");

let activeWhitelistPanelSyncPromise = null;
const whitelistPanelSyncState = new Map();

function buildWhitelistPanelSyncFingerprint(runtime) {
  return JSON.stringify({
    ...
    identifierLabel: runtime?.settings?.identifier_label || "",
  });
}

function shouldSyncWhitelistPanel(runtime) {
  const fingerprint = buildWhitelistPanelSyncFingerprint(runtime);
  const cachedState = whitelistPanelSyncState.get(runtime.guildId);
  const now = Date.now();
...
function markWhitelistPanelSyncState(guildId, fingerprint) {
  whitelistPanelSyncState.set(guildId, {
    fingerprint,
    lastVerifiedAt: Date.now(),
  });
}

function pruneWhitelistPanelSyncState(runtimes) {
  const activeGuildIds = new Set(
    Array.isArray(runtimes) ? runtimes.map((runtime) => runtime.guildId) : [],
  );

  for (const guildId of whitelistPanelSyncState.keys()) {
    if (!activeGuildIds.has(guildId)) {
      whitelistPanelSyncState.delete(guildId);
    }
  }
}
...
function messageLooksLikeWhitelistPanel(message) {
  ...
  walkComponents(message.components, (component) => {
    ...
    if (customId === CUSTOM_IDS.startWhitelist) {
...
async function fetchExistingWhitelistPanelMessage(channel, storedMessageId = null) {
  if (storedMessageId) {
  ...
  return recentMessages.find((message) => messageLooksLikeWhitelistPanel(message)) || null;
}

async function syncWhitelistPanelForRuntime(client, runtime) {
  ...
  const payload = buildWhitelistPanelPayload({
    settings: runtime.settings,
    guildName: guild.name,
  });

  const existingMessage = await fetchExistingWhitelistPanelMessage(
    channel,
  ...
  if (existingMessage) {
    ...
    if (runtime.settings.panel_message_id !== existingMessage.id) {
      await updateGuildWhitelistPanelMessageId(runtime.guildId, existingMessage.id);
  ...
  await updateGuildWhitelistPanelMessageId(runtime.guildId, sentMessage.id);
...
async function syncAllWhitelistPanels(client) {
  if (activeWhitelistPanelSyncPromise) {
    return activeWhitelistPanelSyncPromise;
  }

  activeWhitelistPanelSyncPromise = (async () => {
    const runtimes = await getConfiguredWhitelistGuildRuntimes();
    const applied = [];
    const skipped = [];
    pruneWhitelistPanelSyncState(runtimes);
    ...
    for (const runtime of runtimes) {
      ...
      const syncDecision = shouldSyncWhitelistPanel(runtime);
      ...
      try {
        const result = await syncWhitelistPanelForRuntime(client, runtime);
        markWhitelistPanelSyncState(runtime.guildId, syncDecision.fingerprint);
        applied.push(result);
      } catch (error) {
        skipped.push({
          guildId: runtime.guildId,
  ...
  try {
    return await activeWhitelistPanelSyncPromise;
  } finally {
    activeWhitelistPanelSyncPromise = null;
  }
}

module.exports = {
  syncAllWhitelistPanels,
  syncWhitelistPanelForRuntime,
};
